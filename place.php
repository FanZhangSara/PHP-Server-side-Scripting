...
if(isset($_POST["keywordvalue"])){
  $keywordvalue =urlencode($_POST["keywordvalue"]);
...
//AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4 

//
    

    if($_POST["choose"]!="here"){
      $url = 'https://maps.googleapis.com/maps/api/geocode/json?address='.urlencode($_POST["locvalue"]).'&key=AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4';
  ...
  $dataurl = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location='.$lat.','.$lon.'&radius='.$dis.'&type='.$selectvalue.'&keyword='.$keywordvalue.'&key=AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4';
  ...
  // $dataurl = 'https://maps.googleapis.com/maps/api/place/nearbysearch/json?location=34.0266,-118.2831&radius=16093.44&type=default&keyword=usc&key=AIzaSyBix1e20sK7gC5h48EvDc7SbRhVdn5GH8U'; &radius=16093.44&
    ...
    foreach ($datares as $infoone) {
      $info = array('icon'=>$infoone["icon"],'name'=>$infoone["name"],'vicinity'=>$infoone["vicinity"],'id'=>$infoone["place_id"],'latnow'=>$infoone["geometry"]["location"]["lat"],'lonnow'=>$infoone["geometry"]["location"]["lng"]);
...
if(isset($_POST["varplace"])){
  ...
  $dataurl ='https://maps.googleapis.com/maps/api/place/details/json?placeid='.$varplace.'&key=AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4';
  ...
  $data = file_get_contents($dataurl);
    ...
    if(array_key_exists("photos", $datares)){
      ...
      foreach ($photodetail as $infoone) {
        ...
        $url = 'https://maps.googleapis.com/maps/api/place/photo?maxwidth='.$infoone['width'].'&photoreference='.$infoone['photo_reference'].'&key=AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4';
...
if(isset($_POST["varmap3"])){
  ...
  $dataurl ='https://maps.googleapis.com/maps/api/geocode/json?address='.urlencode($_POST["varmap3"]).'&key=AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4';
  $data = file_get_contents($dataurl);
  $obj = json_decode($data, true);
  ...
  <script>
    ...
    //         });
        ...
        function isVaild(){
        	...
        	var value = true;
        	var reg = new RegExp("^((0|[1-9][0-9]*)|(\\d+(\\.\\d+)?))$");
         ...
         //show all tablelist  ^\d+(.\d+)?$ ^[1-9]\d.\d|0.\d[1-9]\d|0?.0+|0$  ^(0|[1-9][0-9]*)$ ^\d+(.\d+)?$
              ...
              // if () {
                ...
                if(myObj.length==0){
                  ...
                  for(var i=0; i<myObj.length;i++){
                    ...
                    html_text += "<tr class=\"tabletr2\"><td><img  src=\""+myObj[i].icon+"\" width=30px, height=25px></td><td style=\"cursor:pointer;\" onclick=\"showdetail(\'" + myObj[i].id + "\')\">"+myObj[i].name+"</td><td><div style=\"cursor:pointer;\" class=\"uppermap\" style='display:block' onclick=\"initMap(\'" + myObj[i].latnow +  "\',\'" +myObj[i].lonnow +  "\',\'"+ i +"\',\'"+latupper+"\',\'"+lonupper+"\')\">"+myObj[i].vicinity+"</div><div id=\"waydiff"+i+"\"  style='z-index:5;position:absolute'></div><div id=\"map"+i+"\" style='display:none;margin-top:0px;z-index:4;width:400px;height:300px;position:absolute'></div></td></tr>";
                    //alert(myObj[i].id);
                  }
              }
...
//review all the detail and photo details var1
          ...
          if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
                ...
                if(myObj.arr1.length==0){
                  ...
                  for( var i = 0; i < myObj.arr1.length;i++){
                    ...
                    html_text +="<tr style=\"height:30px\"><td style=\"height:30px\"><div><img src=\""+myObj.arr1[i].profile_photo_url+"\"><b>"+myObj.arr1[i].author_name+"</b></div></td></tr>";
                ...
                if(myObj.arr.length==0){
                  ...
                  for( var i = 0; i < myObj.arr.length;i++){
                    //html_text +="<tr><td>"+myObj.arr1[i].profile_photo_url+myObj.arr1[i].author_name+"</td></tr>";
                    html_text +="<tr><td style=\"padding-top:20px;padding-bottom:20px\"><div><a href=\""+myObj.arr[i]+"\" target=\"blank\"><img class=\"reviewimg\" src=\""+myObj.arr[i]+"?"+Math.random()+"\" width=700px, height=500px></a></div></td></tr>";
                  }
                // for( var i = 0; i < myObj.arr.length;i++){
                //   //html_text +="<tr><td>"+myObj.arr1[i].profile_photo_url+myObj.arr1[i].author_name+"</td></tr>";
        ...
        function initMap(lat,lon,i,latupper,lonupper) {
          // var lat;
          // var lon;
          ...
          if(document.getElementById('map'+i).style.display=="none"){
            ...
            document.getElementById('waydiff'+i).style.display="none";
              ...
              //alert(xmlhttp.responseText);
                  ...
                  var html_text = "";
                  html_text += "<div class=\'wayclass\' style=\"cursor:pointer;\" onclick=\"calcRouteW(\'"+lat+ "\',\'"+lon+ "\',\'"+latupper+"\',\'"+lonupper+"\')\">Walk there</div>";
                  html_text += "<div class=\'wayclass\' style=\"cursor:pointer;\" onclick=\"calcRouteB(\'"+lat+ "\',\'"+lon+ "\',\'"+latupper+"\',\'"+lonupper+"\')\">Bike there</div>";
                  html_text += "<div class=\'wayclass\' style=\"cursor:pointer;\" onclick=\"calcRouteD(\'"+lat+ "\',\'"+lon+ "\',\'"+latupper+"\',\'"+lonupper+"\')\">Drive there</div>";
        ...
        //function initMap(lat,lon,i,latupper,lonupper) {
          ...
          // html_text += "<div class=\'wayclass\' onclick=\"calcRouteB(\'"+lat+ "\',\'"+lon+ "\',\'"+latupper+"\',\'"+lonupper+"\')\">Bike there</div>";
          // html_text += "<div class=\'wayclass\' onclick=\"calcRouteD(\'"+lat+ "\',\'"+lon+ "\',\'"+latupper+"\',\'"+lonupper+"\')\">Drive there</div>";
      ...
      function calcRouteB(lat,lon,latupper,lonupper){
      ...
      function calcRouteD(lat,lon,latupper,lonupper){
  ...
  <script async defer src="https://maps.googleapis.com/maps/api/js?key=AIzaSyBhGwaH5KuFI4AgB2Yu4VgKcHy29CtskW4&callback=initMap">
    </script>
